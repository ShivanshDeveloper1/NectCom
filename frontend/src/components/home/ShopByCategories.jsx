...
export const ShopByCategories = ({ products = [] }) => {
  const safeProducts = Array.isArray(products) ? products : [];

  // Group products by category directly from props
  const categoryMap = safeProducts.reduce((acc, prod) => {
    if (!prod.category) return acc;
    const catKey = prod.category.trim();
    if (!acc[catKey]) {
      acc[catKey] = {
        name: catKey,
        slug: catKey.toLowerCase().replace(/\s+/g, '-'),
        image: prod.image || '',
        count: 0,
      };
    }
